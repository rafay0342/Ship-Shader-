...
export const ShipOverlay: React.FC = () => {
    ...
    // Local ref to store interpolated DNA values
    const lerpedECRef = useRef<Record<string, number>>({});
    ...
    useEffect(() => {
        ...
        const render = (t: number) => {
            ...
            const lerpEC = lerpedECRef.current;
            const DNA_LERP_SPEED = 6.0; // Slightly slower for biological/fluid shifting feel

            // Process interpolation for each DNA parameter
            const keys = Object.keys(ec);
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                const targetValue = (ec as any)[key] as number;
                
                if (lerpEC[key] === undefined) {
                    lerpEC[key] = targetValue;
                } else {
                    lerpEC[key] += (targetValue - lerpEC[key]) * DNA_LERP_SPEED * dt;
                }
            }
            
            gl.uniform1f(locs.uComplexity, lerpEC.complexity);
            gl.uniform1f(locs.uFold1, lerpEC.fold1);
            gl.uniform1f(locs.uFold2, lerpEC.fold2);
            gl.uniform1f(locs.uFold3, lerpEC.fold3);
            gl.uniform1f(locs.uScale, lerpEC.scale);
            gl.uniform1f(locs.uStretch, lerpEC.stretch);
            gl.uniform1f(locs.uTaper, lerpEC.taper);
            gl.uniform1f(locs.uTwist, lerpEC.twist);
            gl.uniform1f(locs.uAsymmetryX, lerpEC.asymmetryX);
            gl.uniform1f(locs.uAsymmetryY, lerpEC.asymmetryY);
            gl.uniform1f(locs.uAsymmetryZ, lerpEC.asymmetryZ);

            // New Bias Uniforms
            gl.uniform1f(locs.uTwistAsymX, lerpEC.twistAsymX);
            gl.uniform1f(locs.uScaleAsymX, lerpEC.scaleAsymX);
            gl.uniform1f(locs.uFold1AsymX, lerpEC.fold1AsymX);
            gl.uniform1f(locs.uFold2AsymX, lerpEC.fold2AsymX);
