...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const [shipConfig, setShipConfig] = useState<ShipConfig>({
      ...
      translucency: 1.0,
      modulations: [
          // Default Shape-Shifting patches
          { id: 'ship-mod-1', enabled: true, source: 'turningSigned', target: 'asymmetryX', amount: 0.6 },
          { id: 'ship-mod-2', enabled: true, source: 'speed', target: 'stretch', amount: 0.4 },
          { id: 'ship-mod-3', enabled: true, source: 'pitch', target: 'taper', amount: -0.3 },
          { id: 'ship-mod-4', enabled: true, source: 'turningSigned', target: 'fold1AsymX', amount: 0.4 }
      ]
  });
